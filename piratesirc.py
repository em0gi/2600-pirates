...
def parse_mirc_message(message):
    ...
    current_text = ''
    state = {
        'fg': 'default',      # Foreground color number or 'default'
        'bold': False,
        'underline': False
    }
    i = 0
...
class MyIRCClient(irc.client.SimpleIRCClient):
    ...
    def on_all_raw_messages(self, connection, event):
        ...
        # Forward raw message to all connected service clients
        ...
        if ":irc.znc.in 464" in full_message and "Password required" in raw_message:
            ...
            # Manually parse the raw message
    ...
    try:
        while not stop_event.is_set():
            try:
                ...
                if not data:
                    break
                # Decode and remove leading/trailing whitespace, including \r and \n
...
def navigate_history(direction):
    global history_index
    if direction == 'up':
        if history_index == -1 and message_history:  # Start at the latest message
            history_index = len(message_history) - 1
        elif history_index > 0:  # Move to previous message
            history_index -= 1
        else:
            return  # Stay at the first message
    elif direction == 'down':
        if history_index != -1:
            if history_index < len(message_history) - 1:  # Move to next message
                history_index += 1
            else:  # At the end, clear input and reset
                history_index = -1
                entry.delete(0, END)
                return
        else:
...
def update_gui():
    """Update the GUI, keeping user-specific backgrounds and applying mIRC text formatting."""
    while not incoming_queue.empty():
        ...
        if event['type'] == 'message':
            ...
            # Insert the entire line with the user background
            ...
            # Apply mIRC formatting to message text
            message_start = f"{start_idx}+{len(padding) + len(username) + 3}c"  # After '   '
...
def open_wordel_window():
    ...
    def search_wordel():
        """Process inputs and display matching words in the output area."""
...
# New functions for right-click feature in entry field
def add_command_to_tab(tab_name, new_command):
    """
    Append a new command to the specified tab section in commands.txt.

    Args:
        tab_name (str): The name of the tab (e.g., "General").
        new_command (str): The command text to add.
        ...
        for i, line in enumerate(lines):
            if line.strip().startswith('[') and line.strip().endswith(']'):
                ...
                if current_tab == tab_name:
                    # Find the end of this tab's section
                    for j in range(i + 1, len(lines)):
                        if lines[j].strip().startswith('[') and lines[j].strip().endswith(']'):
                            insert_index = j  # Insert before the next section
                            break
                    else:
                        insert_index = len(lines)  # Insert at file end if last section
                    break
...
def add_item_to_tab(tab_name):
    ...
    Args:
        tab_name (str): The name of the tab to add the command to.
    """
...
def on_entry_right_click(event):
    """
    Display a context menu with tab names and 'Add item' submenus on right-click.
...
if __name__ == '__main__':
    ...
    # Initialize command tabs with default column count
    create_command_tabs(current_column_count)
    ...
    # Entry field spanning columns 0 and 1 with history bindings and right-click
    entry = tk.Entry(root)
    ...
    entry.bind('<Button-3>', on_entry_right_click)  # Bind right-click event
